...
/*
 * Encode:UTF-8 Author:zhiming.xu 多线程的断点下载程序，根据输入的url和指定线程数，来完成断点续传功能。
...
public class MultiTheradDownLoad
{
    ...
    public void downloadPart()
    {
        ...
        filename = filepath.substring(filepath.lastIndexOf('/') + 1, filepath.contains("?")
     ...
     * rantmpfile.seek()跳转到一个位置的目的是为了让各个断点存储的位置尽量分开。 这是实现断点续传的重要基础。
     */
        ...
        try
        {
            if (tmpfile.exists())
            {
                ...
                for (int i = 0; i < threadNum; i++)
                {
                    ...
                    System.out.println("thre thread" + (i + 1) + " startPos:" + startPos[i]
                            + ", endPos: " + endPos[i]);
                }
            }
            else
    ...
    class DownLoadThread implements Runnable
    {
        ...
        public DownLoadThread(long startPos, long endPos, MultiTheradDownLoad task, int id,
                File tmpfile, CountDownLatch latch)
        {
