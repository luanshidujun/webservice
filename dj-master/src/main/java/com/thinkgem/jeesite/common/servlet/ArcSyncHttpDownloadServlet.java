...
import com.thinkgem.jeesite.test.CommonUtil;
...
public class ArcSyncHttpDownloadServlet extends HttpServlet
{
    ...
    {
        String filepath = request.getRequestURI().replace("arcSyncHttp", "userfiles");
...
//        File downloadFile = new File(path);
        ...
        /**
         * 如果设设置了Content -Length，则客户端会自动进行多线程下载。如果不希望支持多线程，则不要设置这个参数。 响应的格式是: Content - Length:
         * [文件的总大小] - [客户端请求的下载的文件块的开始字节]
        ...
        try
        {
            ...
            response.setContentType(CommonUtil.setContentType(downloadFile.getName())); // set the
                                                                                        // MIME
                                                                                        // type.
            ...
            try
            {
                switch (rangeSwitch)
                {
                    ...
                    case 2 :
                    { // 针对 bytes=27000-39000 的请求
                        raf.seek(pastLength); // 形如 bytes=1275856879-1275877358 的客户端请求，找到第
                                              // 1275856879 个字节
                        int n = 0;
